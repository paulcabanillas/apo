...
// Fecha de Creaci�n    : 15/12/2003
// Autor                : Area de desarrollo de sistemas.
// Objetivo             : Menu principal del modulo de Aportes
// Actualizaciones      :
// HPC_201501_APO       : 06/01/2015 - Cambio de DCOM a Socket
// HPC_201801_APO       : : Control de Acceso al Modulo; Al ingresar a los m�dulos de Front;
...
begin
   Errormsg('M�dulo de Aportes', ' No se Puede Eliminar este Registro ');
end;
...
begin
  ...
  Try
    ...
    mtx.UsuarioSQL.ADD(sSQL) ;
   	Mtx.Titulo 				  := 'Proyecci�n de Intereses y Bonus';
...
//Control de Acceso al Modulo; Al ingresar a los m�dulos de Front; implementar un control que no permita acceder al m�dulo sino est� inscrito en el grupo asignado al m�dulo
...
begin
  //BloquearMenuPrincipal();
   ...
   Caption:='M�dulo de Aportes - Usuario: '+ DM1.DisplayDescrip('prvTGE','TGE006','USERNOM',sSQL,'USERNOM');
...
begin
  ...
  Try
    ...
    Mtx.DComC           := DM1.DCOM1;
    Mtx.Module          := DM1.wModulo;
    Mtx.Titulo 		:= 'Cambio de Operaci�n';
    Mtx.OnInsert 	:= nil;
    Mtx.OnEdit 		:= nil;
...
begin
   ...
   Else
    begin
     ShowMessage( 'Para poder continuar, es necesario que actualice la Versi�n');
     DM1.DCOM1.Connected := False;
     Application.Terminate;
    end;
end;
...
//wmc xxx   FToolFusion   := TFToolFusion.Create(Self);
   ...
   Try
    ...
    Mtx.Titulo 	        := 'Redistribuci�n de Unidades de Proceso';
//WMC 666    Mtx.OnCreateMant    := ToolFusionCreate;
    ...
    Mtx.SectionName     := 'Redistribuci�n de Unidades de Proceso';
    Mtx.FileNameIni     := '\SolAportes.ini';
    Mtx.NoVisible.Clear;
    //Mtx.Colors.Add('ASOID;clTeal;clWhite;;;APOFDEV%=%S%;');
    //Mtx.Colors.Add('ASOID;clRed;clWhite;;;APOFRECL%=%S%AND%APOFDEV%<>%S%;');
...
begin
{
   ...
   If Question('Confirmar', 'Esta Seguro de Eliminar La Fusi�n '+ #13+#13+
   ...
   Begin
      if xdiasfaltantes<=0 then
         ...
         if xdiasfaltantes=1 then
            MessageDlg('*** FALTA 1 D�A PARA EL VENCIMIENTO DE SU PASSWORD. ***'#13'                            SE SUGIERE CAMBIARLA AHORA', mtCustom, [mbOk], 0)
         else
            MessageDlg('*** FALTAN '+IntToStr(xdiasfaltantes)+' D�AS PARA EL VENCIMIENTO DE SU PASSWORD. ***'#13'                            SE SUGIERE CAMBIARLA AHORA', mtCustom, [mbOk], 0);
      DM1.cdsUser.Close;
      DM1.cdsUser.DataRequest('SELECT * FROM TGE006 WHERE USERID='''+DM1.wUsuario+'''');
      DM1.cdsUser.Open;
