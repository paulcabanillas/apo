...
uses APODM,MsgDlgs, APO206, APO207, APO208, APO209, APO205, APO210, APO2A1,
  APO2A2, APO212;
{$R *.DFM}
...
begin
    ...
    Else
      Begin
        ...
        DM1.cdsAso.Insert;
        DM1.cdsAso.FieldByName('ASOID').AsString:=dm1.StrZero(DM1.MaxSQL('APO201','ASOID', ''),11);
...
begin
  ...
  If length(edtSexo.Text)=0 Then Begin
    ...
    exit;
  End;
  If length(edtNacionalidad.Text)=0 Then Begin
...
begin
     If (length(dbeNroDNI.Text)<8) And (length(dbeNroDNI.Text)>8) Then Begin
...
procedure TFAsociados.dblcdCodigoCloseUp(Sender: TObject; LookupTable,
...
begin
     If (length(dbeNroRUC.Text)<11) And (length(dbeNroRUC.Text)>11) Then Begin
