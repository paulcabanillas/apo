...
begin
  ...
  Begin
    xSql := 'SELECT ASOID, ASOAPENOMDNI FROM APO201 WHERE ASODNI = '+QuotedStr(Copy(DM1.cdsQry4.FieldByName('CODIDEBCO').AsString,7,8));
    DM1.cdsQry.Close;
    DM1.cdsQry.DataRequest(xSql);
    DM1.cdsQry.Open;
    DM1.cdsQry4.Edit;
    DM1.cdsQry4.FieldByName('ASOAPENOMDNI').AsString := DM1.cdsQry.FieldByName('ASOAPENOMDNI').AsString;
    DM1.cdsQry4.FieldByName('ASOID').AsString := DM1.cdsQry.FieldByName('ASOID').AsString;
    DM1.cdsQry4.FieldByName('DES').AsString := 'ASOID Y CAMPO DE RETORNO DIFERENTES';
    DM1.cdsQry4.Next;
  End;
End;
...
begin
  xasoid := DM1.StrZero(DM1.cdsQry4.FieldByname('ASOID').AsString, 30);
  If MessageDlg('¿ Actualizar registro con los datos de derrama ?',mtConfirmation, [mbYes, mbNo], 0) = mrYes then
  Begin
    xSql := 'UPDATE APO_CUE_REC_BAN_CRE_DET SET CAMRET = '+QuotedStr(xasoid)
    +' WHERE CAMRET IS NULL AND NUMOPE = '+QuotedStr(DM1.cdsQry4.FieldByName('NUMOPE').AsString);
    ...
    +' FROM APO_CUE_REC_BAN_CRE_DET A, APO201 B WHERE FECPAG = '+QuotedStr(xfecpro)
