...
uses
  ...
  Wwdatsrc, ppPrnabl, ppClass, ppCtrls, ppBands, ppCache, ppComm, ppRelatv,
  ppProd, ppReport, ppStrtch, ppMemo, Spin;

type
  TFCaptaAportesWMC = class(TForm)
    pnlCabecera: TPanel;
...
var
  FCaptaAportesWMC : TFCaptaAportesWMC;
...
procedure TFCaptaAportesWMC.Z2bbtnCancelClick(Sender: TObject);
begin
   Close;
end;

procedure TFCaptaAportesWMC.dblcdCodigoExit(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dbgAportesDblClick(Sender: TObject);
var
...
procedure TFCaptaAportesWMC.bbtnIngresaAporteClick(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.Z2bbtnAceptarClick(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcSituacionExit(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcdUSEExit(Sender: TObject);
begin
   DM1.cdsUse.SetKey;
...
procedure TFCaptaAportesWMC.FormClose(Sender: TObject; var Action: TCloseAction);
begin
...
procedure TFCaptaAportesWMC.bbtnAceptaCabClick(Sender: TObject);
var
   xSQL : String;
begin
...
procedure TFCaptaAportesWMC.z2bbtnNuevoClick(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.FormKeyPress(Sender: TObject; var Key: Char);
begin
   If (Key = #13) Then
    begin
     Key := #0;
...
procedure TFCaptaAportesWMC.edtTransaccionExit(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcTransaccionExit(Sender: TObject);
begin
   xWhere:= 'TRANSID = '+ QuotedStr(dblcTransaccion.Text) ;
   edtTransaccion.Text := DM1.DisplayDescrip('prvTGE','APO118','TRANSDES',xWhere,'TRANSDES');
end;

procedure TFCaptaAportesWMC.dblcdCodigoDropDown(Sender: TObject);
begin
   DM1.FiltraCDS(DM1.cdsAsociados1, 'Select ASOID, ASOCODMOD, ASOAPENOM From APO201 Where USEID ='+QuotedStr(dblcdUSE.Text));
end;
procedure TFCaptaAportesWMC.fcShapeBtn2Click(Sender: TObject);
begin
   close ;
   Exit ;
   DM1.cdsTransacciones.Close;
...
procedure TFCaptaAportesWMC.dblcdUProExit(Sender: TObject);
begin
   DM1.cdsUPro.SetKey;
...
procedure TFCaptaAportesWMC.dblcdUPagoExit(Sender: TObject);
begin
   sSQL := 'Select * '+
...
procedure TFCaptaAportesWMC.dblcMonedaExit(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.wwDBGrid1IButtonClick(Sender: TObject);
var
   Present: TDateTime;
...
procedure TFCaptaAportesWMC.fcShapeBtn3Click(Sender: TObject);
begin
//   if bF_Efectivo then
      MostrarPendientes ;
...
procedure TFCaptaAportesWMC.dbeMontoExit(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dbeMontoKeyPress(Sender: TObject; var Key: Char);
var
   sCadBuscar : String ;
   i : integer  ;
   j : integer  ;
...
procedure TFCaptaAportesWMC.spdAnoKeyPress(Sender: TObject; var Key: Char);
begin
   If Key In [' ', 'A'..'Z', 'a'..'z',
              '°', '|', '¬', '!', '"', '#', '$', '%', '&', '/', '(', ')',
              '=', '?', '\', '¡', '¿', '@', '¨', '´', '*', '+', '~', '[',
...
procedure TFCaptaAportesWMC.dbgAportesKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TFCaptaAportesWMC.FormShow(Sender: TObject);
var
   strPeriodo : String ;
begin
   xCrea := True;
...
procedure TFCaptaAportesWMC.RecuperaFechaNombramiento;
var
     iAno , iMes , iDia : Word ;
begin
     strAnoIni := '';
...
procedure TFCaptaAportesWMC.bbtnCierraImpClick(Sender: TObject);
begin
    fcShapeBtn3.OnClick(fcShapeBtn3);
end;

procedure TFCaptaAportesWMC.InsertaAportesPendientes;
var
...
procedure TFCaptaAportesWMC.InsertaHuecos;
var
...
procedure TFCaptaAportesWMC.dbgDetAportesKeyPress(Sender: TObject;
  var Key: Char);
var
   sCadBuscar : String ;
   i : integer  ;
...
procedure TFCaptaAportesWMC.cb_okChange(Sender: TField);
var
    evt_CB_OK2   ,
...
procedure TFCaptaAportesWMC.dbgDetAportesCellChanged(Sender: TObject);
begin
   if dbgdetaportes.getactivefield.fieldname = 'TRANSMTO' THEN
...
procedure TFCaptaAportesWMC.cb_ok2Validate(Sender: TField);
begin
...
procedure TFCaptaAportesWMC.blanquea_parcial;
var
  cdsClone : Twwclientdataset ;
  i : integer ;
begin
...
procedure TFCaptaAportesWMC.cb_ok2Change(Sender: TField);
var
   evt_TRANSMTO : TFieldNotifyEvent ;
begin
   blanquea_parcial ;
...
procedure TFCaptaAportesWMC.transmtoValidate(Sender: TField);
begin
...
procedure TFCaptaAportesWMC.transmtoChange(Sender: TField);
begin
   if dbgdetAportes.getActiveField.FieldName = 'TRANSMTO' then
   begin
...
procedure TFCaptaAportesWMC.ConfiguraGridAdicion;
begin
   ...
   dbgDetAportes.Selected.Add('CB_OK2'#9'3'#9'Parcial'#9'F');
...
procedure TFCaptaAportesWMC.ConfiguraGridEdicion;
begin
   dbgDetAportes.Selected.Clear;
   dbgDetAportes.Selected.Add('TRANSANO'#9'6'#9'Año'#9'T');
   dbgDetAportes.Selected.Add('TRANSMES'#9'4'#9'Mes'#9'T');
...
procedure TFCaptaAportesWMC.LimpiaCamposEdicion;
begin
   dblcFormaPago.Clear;
   dbeFormaPago.Clear;
   dblcBanco.Clear;
...
procedure TFCaptaAportesWMC.bbtnAceptarClick2(Sender: TObject);
var
...
procedure TFCaptaAportesWMC.ConfiguraAdicion;
begin
...
procedure TFCaptaAportesWMC.dblcFormaPagoChange(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcBancoChange(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcAgenciaChange(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.dblcCtaCteExit(Sender: TObject);
begin
   if not dblcCtaCte.LookUpTable.Locate(dblcCtaCte.Lookupfield,VarArrayOf([trim(dblcCtaCte.Text)]),[]) then
      dblcCtaCte.Clear ;
end;

procedure TFCaptaAportesWMC.dblcFormaPagoExit(Sender: TObject);
begin
  if trim(dbeFormaPago.Text) = '' then
     dblcFormaPago.Clear ;
end;

procedure TFCaptaAportesWMC.dblcBancoExit(Sender: TObject);
begin
  if trim(dbeBanco.Text) = '' then
     dblcBanco.clear ;
end;

procedure TFCaptaAportesWMC.dblcAgenciaExit(Sender: TObject);
begin
  if trim(dbeAgencia.Text) = '' then
     dblcAgencia.Clear ;
end;

procedure TFCaptaAportesWMC.dbeMontoEnter(Sender: TObject);
begin
   if cdsQry2.State in [dsinsert,dsedit] then
      cdsqry2.Post ;
end;

procedure TFCaptaAportesWMC.blanquea_detalle;
var
  bmk : TBookMarkStr ;
  i : integer ;
begin
...
procedure TFCaptaAportesWMC.PonEventosCampos;
begin
...
procedure TFCaptaAportesWMC.QuitaEventosCampos;
begin
...
procedure TFCaptaAportesWMC.recalcula_Detalle;
var
  bmk : TBookMarkStr ;
  currMonto : currency ;
  i :integer ;
...
function TFCaptaAportesWMC.QuitaComas(strNumero: String):Currency;
var
    iPos : Integer ;
begin
...
procedure TFCaptaAportesWMC.ValidaINgresoAportes;
begin
   if cdsQry2.State in [dsInsert,dsEdit] then
      cdsQry2.Post ;
   If (trim(dbeFormaPago.Text) = '') Then
...
function TFCaptaAportesWMC.Sumariza:Currency;
var
  cdsClone : Twwclientdataset ;
  i : integer ;
begin
...
function TFCaptaAportesWMC.RecuperaPeriodo;
var
  iAno , iMes , iDia : Word ;
begin
...
procedure TFCaptaAportesWMC.DesHabilitaPanelEdicion;
begin
...
procedure TFCaptaAportesWMC.HabilitaPanelEdicion;
begin
...
procedure TFCaptaAportesWMC.bbtnNuevoClick(Sender: TObject);
begin
//   if bF_Efectivo then
   if dbgAportes.DataSource.DataSet.RecordCount = 0 then
   begin
...
procedure TFCaptaAportesWMC.ValidarAdicionAportes;
begin
{ wmc
...
procedure TFCaptaAportesWMC.PonEventosCabecera;
begin
...
procedure TFCaptaAportesWMC.QuitaEventosCabecera;
begin
...
procedure TFCaptaAportesWMC.FormCreate(Sender: TObject);
var
  sUsuario  : String ;
begin
...
procedure TFCaptaAportesWMC.FormDestroy(Sender: TObject);
begin
...
function TFCaptaAportesWMC.RecuperaCorrelativo: String;
var
  sANO   : String ;
  sMES   : String ;
  sSQL   : String ;
...
procedure TFCaptaAportesWMC.MostrarPendientes;
begin
   dbgDetAportes.datasource.DataSet.Filter := '' ;
   dbgDetAportes.datasource.DataSet.Filtered := fALSE ;
end;

procedure TFCaptaAportesWMC.QuitarPendientes;
begin
...
procedure TFCaptaAportesWMC.Actualiza;
Const
...
procedure TFCaptaAportesWMC.ActuCaja301;
Const
...
procedure TFCaptaAportesWMC.RecUltTipoCambio(var xFecha: string);
var
  xSQL : String ;
begin
...
procedure TFCaptaAportesWMC.AsignaAnoMes(var sAno, sMes: String);
var
  Year, Month, Day : Word;
begin
...
procedure TFCaptaAportesWMC.bbtnImprimirClick(Sender: TObject);
var
 x : String ;
...
function TFCaptaAportesWMC.NumLetra(const mNum: Currency; const iIdioma, iModo: Smallint): String;
const
  iTopFil: Smallint = 6;
  iTopCol: Smallint = 10;
...
procedure TFCaptaAportesWMC.ppReport1PreviewFormCreate(Sender: TObject);
begin
...
procedure TFCaptaAportesWMC.bbtnAceptarPeriodoInicialClick(Sender: TObject);
var
   Present: TDateTime;
   nYear, nMonth, nDay : word;
   bmk : TBookmark ;
...
procedure TFCaptaAportesWMC.bbtnSalirPeriodoInicialClick(Sender: TObject);
begin
  //
...
procedure TFCaptaAportesWMC.SpeAnnosKeyPress(Sender: TObject; var Key: Char);
begin
       if key in ['.','-'] then
          key := #0 ;
end;

procedure TFCaptaAportesWMC.fcShapeBtn5Click(Sender: TObject);
begin
  bbtnSalirPeriodoInicial . OnClick ( bbtnSalirPeriodoInicial );
end;

procedure TFCaptaAportesWMC.ValidaPeriodoInicial;
begin
